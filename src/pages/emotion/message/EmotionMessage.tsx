...
const EmotionMessage = () => {
  ...
  관계적 영역(10점~50점)은 몇 점일지 예상해서 "relational"에 숫자 형태로 저장해주고, 아이 없는 일상에 대한 거부 영역(8점~40점)은 몇 점일지 예상해서 "refusing"에 숫자 형태로 저장해줘.
  마지막으로 부모됨의 필요성 영역(10점~50점)은 몇 점일지 예상해서 "essential"에 숫자 형태로 저장해줘.
  영역별 점수를 다 더해서 "totalScore"에 숫자 형태로 저장해줘. social, sexual, relational, refusing, essential 영역을 다 더한 점수가 totalScore야.

  - emotions: 사용자의 특성을 고려하여 오늘 사용자가 기분 좋았던 일, 안 좋았던 일, 임신을 위해 한 노력, 본인과 배우자에게 해 주고 싶은 말을 토대로 사용자의 감정을 Plutchik의 감정 휠 6가지 범주, "joy", "sadness", "anger", "fear", "surprise", "disgust"로 나누어 각각 비율이 몇 퍼센트인지 숫자 형태로 객체 담아서 저장해줘. 총 합이 100%가 되어야 해.
  ...
  - keywords : 사용자의 오늘 하루 기분 좋았던 일과 사용자의 관심사 ${keywords}를 토대로 사용자가 요즘 어떤 것에 관심이 있는지 명사형, "~하기", 또는 "~하는 것"의 형태로 3가지 뽑아서 배열에 저장해줘. 장소는 어떤 공간인지가 중요하고 지역은 별로 중요하지 않아. 세 가지 키워드가 서로 겹치지 않는 범주였으면 좋겠어. 직장과 관련된 일, 병원이나 관계에 관련된 일은 제외해줘.
  `;
